...
class Manouver(object):
    ...
    def __init__(self,name):
        ...
        self._move_client = SimpleActionClient("/{}/action/pose".format(self.robot_name), PoseAction)     # Server name = robot_name/move_base
        self._move_client.wait_for_server()

        self.pub = rospy.Publisher("/{}/manouvers/out".format(name), drone_events_message, queue_size=10)         # Publisher object
        self.msg = drone_events_message()                                                                         # Message object
    ...
    def move_to(self,x,y,z,theta):
        ...
        '''
        dest = PoseGoal()
        dest.target_pose.header.frame_id = "{}/world".format(self.robot_name)
        dest.target_pose.pose.position.x = x                    # Desired x position
        dest.target_pose.pose.position.y = y                    # Desired y position
        dest.target_pose.pose.position.z = z                    # Desired z position
...
if __name__=="__main__":
    ...
    rospy.Subscriber("manouvers/in", drone_events_message, manouver_event, queue_size=10)            # Topic to receive command events
    # rospy.Subscriber("victim_sensor/out", drone_events_message, victim_event, queue_size=10)         # Topic to receive events from victim sensor
