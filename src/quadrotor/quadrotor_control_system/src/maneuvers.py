...
#         It requires the goal region with at least 3 point, and a start position. 
#         After suspended the robot can reestart the v_search with the resume command.
...
def maneuver_event(msg):
    ...
    #     if (msg.event == "start_v_search") and (exp.state == 'IDLE'):
    #         # Verify paramenters
    #         if len(msg.position) > 3:                                                           # Need at least three points to create a polygon
    #             thread = Thread(target=exp.execute, args=[msg.position])
    #             thread.start()                                                                  # Start v_search
    #         else:
    #             rospy.logwarn("Wrong v_search parameters! Must have at least 3 vertices and 1 start position.")             # Missing parameters
        
    #     elif (msg.event == "suspend_v_search") and (exp.state == 'EXE'):
    #         exp.suspend()
    #     elif (msg.event == "resume_v_search") and (exp.state == 'SUS'):
    #         thread = Thread(target=exp.execute)
    #         thread.start()
    #     elif (msg.event == "reset_v_search") and (exp.state =='ERROR'):
    #         exp.reset()
    #     elif (msg.event == "abort_v_search") and (exp.state == 'SUS'):
    #         exp.abort()
    #     else:
    #         rospy.logwarn("Command not allowed!")
    ...
    #     if (msg.event == "start_safe_land") and (tele.state =='IDLE'):
    #         thread = Thread(target=tele.execute)
    #         thread.start()                                                                      # Start safe_land
    #     elif (msg.event == "reset_safe_land") and (tele.state == 'ERROR'):
    #         tele.reset()
    #     elif (msg.event == "end_safe_land") and (tele.state == 'EXE'):
    #         # safe_land ended
    #         tele.end()
    #     elif (msg.event == "safe_land_error") and (tele.state == 'EXE'):
    #         # Safe land ended
    #         tele.error()
    #     else:
    #         rospy.logwarn("Command not allowed!")
