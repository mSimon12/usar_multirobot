...
#####----- HANDLERS generation methods: ------######################################################################    
    def gen_events_calls(self):
        ...
        if os.stat(filename).st_size == 0:
            ...
            events_file.write("import os\n")   
            events_file.write("import importlib\n")   
            events_file.write("import rospy\n")   
        ...
        for event in self.__events.index:
            ...
            if code not in content:
                ...
                # events_file.write("\n\t\tpass")
                if self.__events.at[event,'controllable'] == True:
                    events_file.write("\n\t\tmsg = " + event + ".module.events_message()")
                    events_file.write("\n\t\tmsg.event = " + event + ".output['ll_event']")
                    # events_file.write("\n\t\tif param:")
                    # events_file.write("\n\t\t\t#"+ event +".msg.info = param[0]")
                    # events_file.write("\n\t\t\t#"+ event +".msg.param = param[1]")
                    # events_file.write("\n\t\t\t#"+ event +".msg.position = param[2]")
                    # events_file.write("\n\t\t\tpass")
                    events_file.write("\n\t\t"+ event +".pub.publish(msg)					#Publish message")
                    events_file.write("\n\t\treturn True")
