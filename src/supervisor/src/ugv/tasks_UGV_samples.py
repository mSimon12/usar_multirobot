...
class UGV_exploration(Task):
    ...
    def __init__(self, param, vs_req = True, gs_req = True):
        super().__init__(param, False, True)

    def next_event(self, states, last_event, event_param = []):
        '''
            Changed to a sequence that does not fit to the models applied
            Exploration sequence: on_vs -> -> st_exp -> (rsm_exp, end_exp) -> -> off_vs     (falta acionar GS)
        '''        
